        ...
        try (var ignored = environment.time(Metrics.FS_OPS)) {
            switch (mode) {
                case "r" -> {
                    ...
                    var reader = getFileSystem().openForRead(path, EncodedReadableHandle::openISO_8859_1);
                    return new Object[]{ new EncodedReadableHandle(reader.get(), reader) };
                ...
                case "w" -> {
                    ...
                    var writer = getFileSystem().openForWrite(path, false, EncodedWritableHandle::openISO_8859_1);
                    return new Object[]{ new EncodedWritableHandle(writer.get(), writer) };
                }
                case "a" -> {
